...
{{-- High Security Locker Card --}}
<div class="dp-card">
    {{-- Section Header --}}
    <div class="flex items-center justify-between pb-5 mb-6 border-b border-[var(--color-dp-border)]">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-xl bg-red-50 flex items-center justify-center">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#e53e3e" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
            </div>
            <div>
                <h2 class="text-[18px] font-bold text-[#1a1a24]">High Security</h2>
                <p class="text-[13px] text-[var(--color-dp-text-muted)]">Store your most important and sensitive credentials securely.</p>
            </div>
        </div>
        <button onclick="openModal('add')" class="flex items-center gap-2 px-4 py-2 bg-red-600 text-white text-[13px] font-semibold rounded-xl hover:bg-red-700 transition-all">
    ...
    <div class="space-y-3">
        ...
        <div class="group relative flex items-center justify-between px-5 py-4 bg-[var(--color-dp-bg)] border border-[var(--color-dp-border)] rounded-2xl hover:border-red-500/30 hover:bg-red-50/50 transition-all duration-200">
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" onclick="togglePassword({{ $credential->id }}, this)" class="text-gray-400 hover:text-red-500 transition-colors p-1" title="Show/Hide">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                    </button>
                    <button type="button" onclick="copyPassword({{ $credential->id }}, this)" class="text-gray-400 hover:text-red-500 transition-colors p-1" title="Copy">
        ...
        <div class="py-10 text-center">
            ...
            <p class="text-[14px] font-semibold text-gray-700">No high-security credentials found.</p>
            <p class="text-[13px] text-gray-400 mt-1">Click 'Add New' to store sensitive logins securely.</p>
        </div>
        @endforelse
    </div>
</div>
        ...
        <form id="credential-form" onsubmit="saveCredential(event)">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="credential-name" required class="w-full px-4 py-2.5 text-[14px] bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400 transition-all">
                </div>
                <div>
                    <label class="block text-[13px] font-bold text-gray-700 mb-1.5">ID / Email *</label>
                    <input type="text" id="credential-login" required class="w-full px-4 py-2.5 text-[14px] bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400 transition-all">
                </div>
                <div>
                    <label class="block text-[13px] font-bold text-gray-700 mb-1.5">Password *</label>
                    <div class="relative">
                        <input type="password" id="credential-password" required class="w-full px-4 py-2.5 text-[14px] bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400 transition-all pr-10">
            ...
            <div class="flex items-center gap-3 justify-end mt-8">
                ...
                <button type="submit" class="px-5 py-2.5 text-[14px] font-semibold text-white bg-red-600 hover:bg-red-700 rounded-xl transition-all flex items-center gap-2">
...
<script>
    ...
    function openModal(mode, data = null) {
        ...
        if(mode === 'edit' && data) {
            ...
            fetch(`/settings/security-high-credentials/${data.id}/password`, {
    ...
    document.getElementById('confirm-delete-btn').addEventListener('click', () => {
        ...
        fetch(`/settings/security-high-credentials/${currentDeleteId}`, {
            method: 'DELETE',
            headers: {
            ...
            if(res.success) {
                if (window.DevOSAlert) window.DevOSAlert.deleted('done successfully :)', 'High security entry deleted.');
    ...
    function saveCredential(e) {
        ...
        const url = id ? `/settings/security-high-credentials/${id}` : '/settings/security-high-credentials';
            ...
            if(res.success) {
                if (window.DevOSAlert) {
                    window.DevOSAlert[id ? 'update' : 'success'](
                        'done successfully :)',
                        id ? 'High security entry updated.' : 'New high security entry added.'
                    );
                }
                window.DevOSNotifications?.refresh();
                setTimeout(() => location.reload(), 400);
    ...
    function togglePin(id) {
        ...
        fetch(`/settings/security-high-credentials/${id}/pin`, {
            method: 'POST',
    ...
    function togglePassword(id, btnIcon) {
        ...
        if (visiblePasswords[id]) {
            ...
            // Fetch and Show
            fetch(`/settings/security-high-credentials/${id}/password`, {
                headers: { 'Accept': 'application/json' }
            }).then(res => res.json()).then(res => {
                if(res.success) {
        ...
        fetch(`/settings/security-high-credentials/${id}/password`, {
            ...
            if(res.success) {
                navigator.clipboard.writeText(res.password).then(() => {
                    ...
                    toast.className = 'fixed bottom-5 right-5 bg-red-900 text-white px-4 py-2 rounded-lg text-sm shadow-xl z-50 animate-fade-in-up';
                    toast.textContent = 'High Security Password copied';
